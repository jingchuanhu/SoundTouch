...
namespace jch {
    ...
    int JchSoundTouch::ProcessData() {
        ...
        ProcessData(buf, shortBufSize, samples);
    }
    ...
    int JchSoundTouch::ProcessData(short *buf, size_t samples, size_t bufferSize) {
        ...
        try {
            ...
            LOGV("jchSoundTouch","JNI exception in SoundTouch::processFile: %s", err);
            errorMsg_ = err;
            return -1;
        }
    ...
    void JchSoundTouch::OnProcessedData(short *buf, size_t samples) {
        ...
        jch::GetEnv()->CallVoidMethod(processCallback_.obj(), processMethodId_);
